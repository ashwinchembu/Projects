...
{
  cout << "inserting 2 " << to_insert->key << endl;
    ...
    cout << "inserted" << endl;
    start->height = calheight(start);
    int m = bf(start);
    //cout << "ceckin code1" << endl;
    int right = bf(start->right);
    int left = bf(start->left);
    //cout << "ceckin code2" << endl;
    if(m == 2 && left == 1){
      //cout << "left" << endl;
      start = lrotation(start);
    }
    else if(m == -2 && right == -1){
      //cout << "right" << endl;
      start = rrotation(start);
    }
    else if(m == -2 && right == 1){
...
{
    if (start == NULL || val == start->key) // tree is empty or we found val
        return start;
