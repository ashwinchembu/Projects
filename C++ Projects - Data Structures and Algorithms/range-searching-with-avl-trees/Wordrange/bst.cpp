...
void BST :: insert(Node* start, Node* to_insert)
{
    if (start == NULL) // in general, this should not happen. We never call insert from a null tree
        return;
    char *com1 = strdup(to_insert->key.c_str());
    char *com2 = strdup(start->key.c_str());
    if (strcmp(com1,com2) <= 0) // inserted node has smaller (or equal) key, so go left
    {
        if(start->left == NULL)
        {
...
{
    char *com1 = strdup(val.c_str());
    char *com2 = strdup(start->key.c_str());
    if (start == NULL || strcmp(com1,com2) == 0) // tree is empty or we found val
        return start;
    if(strcmp(com1,com2) < 0) // val is smaller, so go left
...
int BST :: range(string one, string two){
    return range(root,one,two);
}
int BST :: range(Node* start, string one1, string two2){
    if(start == NULL){
        //cout << "no values" << endl;
        return 0;
    }
    char *com1 = strdup(start->key.c_str());
    char *one = strdup(one1.c_str());
    char *two = strdup(two2.c_str());
    //cout << "at com1: " << com1 << endl;
    if(strcmp(com1,one) == 0 && strcmp(com1,two) == 0){
        //cout << "don't recur" << endl;
        return 1;
    }
    if(strcmp(com1,two) <= 0 && strcmp(com1,one) >= 0){
        //cout << "recurring left and right" << endl;
        return 1 + range(start->left,one1,two2) + range(start->right, one1, two2);
    }
    else if (strcmp(com1,one) < 0){
