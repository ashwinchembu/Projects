...
int main(int argc, char** argv)
{
    ...
    while(getline(input,command)) // get next line of input, store as repeat
    {
        ...
        if (strcmp(op, "lca")){
            Node* temp = myBST.lca(val,20);
            cout << "LCA at "<< temp->key << endl;
        }
        if (strcmp(op,"w")){
            int m = myBST.getMaxWidth();
            cout << "width is" << m << endl;
        }
    ...
    if(test.lca(10,4) != NULL){
    cout << test.lca(10,4)->key << endl; 
    }
    /*
    cout << test.getMaxWidth() << endl;
    */
